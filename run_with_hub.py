...
class GPXVideoOverlay:
    ...
    def __init__(self, csv_path="aligned.csv", map_position="topright", map_scale=0.2, layout="default"):
        ...
        self.map_scale = map_scale        # 小地图宽度占画面宽度的比例
        self.layout = layout  # 新增：布局参数
    ...
    def _preload_icons(self):
        ...
        for name, path in icon_paths.items():
            ...
            if not os.path.exists(path):
                print(
                    f"Warning: Icon file not found at {path}. Display for {name} will be skipped.")
                continue
    ...
    def _draw_path(self, frame, t_idx):
        ...
        for i in range(1, len(pts)):
            cv2.line(frame, tuple(pts[i-1]), tuple(pts[i]), (0, 255, 255), 8)
        ...
        # ==== 4. 绘制小地图边框 ====
        cv2.rectangle(frame, (map_x1, map_y1),
                      (map_x2, map_y2), (255, 255, 255), 2)
    ...
    def _draw_hud(self, frame, t_idx):
        ...
        if self.layout == "default":
            ...
            # 给该矩形画个边框
            # cv2.rectangle(frame, (x0, y0), (x0 + cell_w, y0 + cell_h), (255, 255, 255), 2)
...
if __name__ == "__main__":
    overlay_processor = GPXVideoOverlay(
        csv_path="aligned.csv", map_position="topright", map_scale=0.2, layout="grid9")
    overlay_processor.process_video(
        "video/DJI_20250916214343_0373_D.MP4",
        "output/001_test.mp4",
        max_duration=5  # 只渲染前 10 秒
    )
